...
bool ACMCentipedeLearningCoordinator::InitializeLearningObjects()
{
    ...
    {
        if (!CMAggressiveLearningSnapshot::HasCompleteTrainingSnapshots(ECMAggressiveLearningSnapshotProfile::Centipede, SnapshotDirectory)
            || !CMAggressiveLearningSnapshot::LoadTrainingNetworks(ECMAggressiveLearningSnapshotProfile::Centipede, *Policy, *Critic, SnapshotDirectory))
            return false;
    }
...
void ACMCentipedeLearningCoordinator::LogTrainingProgressIfNeeded()
{
    ...
    if (CurrentTime < NextProgressLogTime || !TrainingEnvironment)
        return;
    NextProgressLogTime = CurrentTime + FMath::Max(ProgressLogIntervalSeconds, 0.1f);
    UE_LOG(LogCMCentipedeLearning, Display,
