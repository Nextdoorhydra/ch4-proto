...
class ACMConveyorDistributorActor;
...
UCLASS(Blueprintable, meta = (DisplayName = "Conveyor Route Controller"))
...
public:
    ...
    UFUNCTION(BlueprintPure, Category = "Chimera|Conveyor|Network")
    int32 GetAutoGeneratedRouteControllerCount() const { return AutoGeneratedRouteControllers.Num(); }

    UFUNCTION(BlueprintPure, Category = "Chimera|Conveyor|Network")
    int32 GetConnectedDistributorCount() const { return ConnectedDistributors.Num(); }
...
protected:
    ...
    UPROPERTY(ReplicatedUsing = OnRep_FirstSegment, EditInstanceOnly, BlueprintReadOnly, Category = "Chimera|Conveyor|Route")
    ...
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Chimera|Conveyor|Network")
    bool bAutoDiscoverConnectedNetwork = true;

    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Chimera|Conveyor|Network", meta = (ClampMin = "1.0", ForceUnits = "cm"))
    float NetworkConnectionTolerance = 25.0f;

    UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Chimera|Conveyor|Network")
    TArray<TObjectPtr<ACMConveyorSplineActor>> AutoGeneratedRouteControllers;

    UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Chimera|Conveyor|Network")
    TArray<TObjectPtr<ACMConveyorDistributorActor>> ConnectedDistributors;
    ...
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Chimera|Conveyor|Detection", meta = (ClampMin = "0.0", ForceUnits = "cm"))
    float HandoffAcceptanceDistance = 25.0f;
...
private:
    ...
    void BuildConnectedNetwork();
    ACMConveyorSplineActor* SpawnAutoRouteController(ACMConveyorSegmentActor* RouteFirstSegment);
    ...
    bool IsPartNearRouteEndpoint(const ACMPartActorBase* PartActor) const;
    void UpdateMovementTimer();
    void StopMovementTimer();
    void UpdateTrackedParts();
    bool TryHandoffToDistributor(ACMPartActorBase* PartActor) const;
    ...
    bool bAutoGeneratedNetworkChild = false;
