	...
	if (!IsDesignTime() && ExtensionPointTag.IsValid())
	{
		...
		if (ULocalPlayer* LocalPlayer = GetOwningLocalPlayer())
		{
			if (APlayerController* PlayerController = LocalPlayer->GetPlayerController(GetWorld()))
			{
				...
				{
					RegisterExtensionPointForPlayerState(LocalPlayer, PlayerState);
				}
			}
		}
	}
...
void UUIExtensionPointWidget::RegisterExtensionPointForPlayerState(ULocalPlayer* LocalPlayer, APlayerState* PlayerState)
{
	if (UUIExtensionSubsystem* ExtensionSubsystem = GetWorld()->GetSubsystem<UUIExtensionSubsystem>())
	{
		TArray<UClass*> AllowedDataClasses;
