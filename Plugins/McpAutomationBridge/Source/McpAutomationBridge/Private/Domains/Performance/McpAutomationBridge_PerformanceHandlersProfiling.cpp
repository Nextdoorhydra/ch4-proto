...
bool HandleProfilingAction(const FPerformanceActionContext& Context)
{
    ...
    if (Context.Lower == TEXT("generate_memory_report"))
    {
        ...
        constexpr int32 MaxReportFiles = 256;
        IFileManager::Get().IterateDirectoryRecursively(
            *ReportDirectory,
            [&ReportFiles](const TCHAR* FilenameOrDirectory, bool bIsDirectory)
            {
                if (!bIsDirectory &&
                    FPaths::GetExtension(FilenameOrDirectory).Equals(
                        TEXT("memreport"), ESearchCase::IgnoreCase))
                {
                    ReportFiles.Add(FilenameOrDirectory);
                }
                return ReportFiles.Num() < MaxReportFiles;
            });
