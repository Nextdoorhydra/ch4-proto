...
현재 N: 이전 룸부터 N/N+1까지 로드·표시를 유지하고 N+2는 숨김 프리로드한다. 더 먼 미래 룸은 아직 로드하지 않는다. 클라이언트도 복제된 CurrentRoomIndex로 로컬 스트리밍을 적용한다. 다음 룸 장애물은 Start Active를 따르며 버튼으로 켤 대상은 false로 둔다. 지나온 룸은 스테이지 맵을 떠날 때까지 유지되므로 메모리와 액터 처리 비용이 누적될 수 있다.

진입 판정은 이벤트를 일으킨 **한 ACMChimera의 활성 몸통 마디 하나 이상**이다. 문 상태와 무관하게 즉시 Commit하여 체크포인트를 등록한다. 선택적으로 지정한 EntryBlockerDoor가 활성 상태라면 Commit 성공 후 닫기 명령만 보내고 완료는 기다리지 않는다. 자동으로 닫을 필요가 없으면 EntryBlockerDoor를 비워둔다. bWaitForDoorClosed 옵션은 제거되었다.
